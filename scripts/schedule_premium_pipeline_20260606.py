...
# Plano de agendamento cronológico para amanhã (6 de Junho de 2026)
PLAN = [
    ("topic_3c63d008c9bda317af", "2026-06-06T09:00:00+01:00"),  # Adoção de IA em Portugal: o dilema da governança interna
    ("topic_aef3422a94b3bacd1e", "2026-06-06T13:00:00+01:00"),  # Anthropic e a Pausa Estratégica na IA
    ("topic_8e074bba70858b0d84", "2026-06-06T16:00:00+01:00"),  # OpenAI propõe plano para resiliência biológica com IA
    ("topic_d5e7049764a437bacf", "2026-06-06T21:00:00+01:00"),  # Cognizant Lança IA Física Soberana
]

def main() -> None:
    load_dotenv()
    print("=== INICIANDO AGENDAMENTO EDITORIAL PREMIUM (2026-06-06) ===")
    ...
    # Segundo: Instagram Carrossel Combinado às 21:00 (apenas posts de instagram existentes)
    print("\n-> A preparar o Carrossel Combinado de Instagram (slides) para as 21:00...")
    instagram_posts_sorted = []
    for topic_id, _ in PLAN:
        inst_posts = [p for p in posts_by_topic[topic_id] if p.channel == "instagram"]
        if inst_posts:
    ...
    if instagram_posts_sorted:
        ...
        carousel_time = "2026-06-06T21:00:00+01:00"
        ...
        if first_inst.status == "scheduled" and first_inst.buffer_post_id and first_inst.buffer_post_id != "manual_buffer_media_required":
            ...
            for p in instagram_posts_sorted:
                ...
                update_final_post_copy(
                    state.final_posts_path,
                    post_id=p.post_id,
                    notes="[2026-06-06] Agendado como parte do Carrossel Combinado de slides das 21:00."
                )
                scheduled_summary.append({
                    "post_id": p.post_id,
                    "channel": "instagram",
    ...
    print("\n=== TABELA DE RESUMO DE AGENDAMENTO (2026-06-06) ===")
