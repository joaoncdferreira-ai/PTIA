...
# Plano de agendamento cronológico para o post das 21:00 de hoje (15 de Junho de 2026)
PLAN_21H = [
    ("topic_ced6c42fbf093528d6", "2026-06-15T21:00:00+01:00"),  # Famílias russas usam IA
]

def main() -> None:
    load_dotenv()
    print("=== INICIANDO AGENDAMENTO EDITORIAL PREMIUM 21H & INSTAGRAM CARROSSEL (2026-06-15) ===")
    state = DashboardState(ROOT / "data")
    
    # 1. Efetuar Backup da base de dados local
    backup_path = state.final_posts_path.with_name(
        f"final_posts.jsonl.bak_before_21h_schedule_{datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    # 2. Filtrar posts em "approved_for_schedule" para o novo tópico de 21h
    target_topic_ids = {topic_id for topic_id, _ in PLAN_21H}
    relevant_21h = [
        post
        for post in posts
    ...
    print(f"-> Detetados {len(relevant_21h)} posts relevantes para o tópico de 21h.")
    
    # Agrupar posts de 21h por tópico
    posts_by_topic = {topic_id: [] for topic_id, _ in PLAN_21H}
    for post in relevant_21h:
        posts_by_topic[post.topic_id].append(post)

    # 3. Preparar e publicar imagens no repositório de assets públicos do Git
    print("-> A copiar e publicar imagens variantes de todos os posts sociais de 21h no Git...")
    social_posts = [p for p in relevant_21h if p.channel in {"linkedin", "x"} and p.image_path]
    public_paths = []
    ...
    # 4. Aguardar disponibilidade pública dos URLs no CDN do GitHub
    if social_posts:
        ...
        if missing:
            ...
            print("-> Sucesso: Todas as imagens de 21h estão publicamente acessíveis na CDN!")

    # 5. Executar agendamento do tópico de 21h por canais (LinkedIn, X, Site)
    print("\n--- INICIANDO PROCESSAMENTO DE AGENDAMENTOS 21H ---")
    
    scheduled_summary = []

    for topic_id, scheduled_time in PLAN_21H:
    ...
    # 6. Instagram Carrossel às 21:00
    print("\n-> A preparar o Carrossel de Instagram para as 21:00...")
    instagram_posts_sorted = []
    original_plan_order = [
        "topic_1ab3dae66e6a9c88ec",  # Visa / OpenAI
        "topic_b35c96b074a01d441f",  # Seguro - IA na Justiça
        "topic_4b3b62db06f6eec116",  # DeepMind - Interação de Agentes
        "topic_ced6c42fbf093528d6",  # Famílias russas usam IA
    ]
    
    # Recarregar posts para pegar todos os Instagram posts aprovados
    posts = load_final_posts(state.final_posts_path)
    ...
    if len(instagram_posts_sorted) == 4:
        ...
        combined_body = "\n\n".join(paragraphs)
        combined_hashtags = "#InteligenciaArtificial #IA #Negocios #Tech #PTIA #Guerra #Seguranca #Justica #Regulacao #Etica"
        combined_sources = "Fontes:\n" + "\n".join(sources)
        
        legend_text = f"{combined_body}\n\n{combined_hashtags}\n\n{combined_sources}".strip()
        
        # Copiar e publicar imagens de Instagram no Git
        print("-> A copiar e publicar imagens do Carrossel de Instagram no Git...")
        inst_public_paths = []
        for p in instagram_posts_sorted:
            path = _copy_image_to_public_site_assets(state, p)
            if path:
                inst_public_paths.append(path)
                
        if inst_public_paths:
            print("   A executar 'git push' para imagens do Instagram...")
            _publish_site_assets_to_git(state, inst_public_paths)
            print("   Push concluído!")
            
            # Aguardar disponibilidade pública das imagens
            print("-> A verificar disponibilidade pública de todas as imagens do Instagram no CDN...")
            missing_inst = list(instagram_posts_sorted)
            for attempt in range(15):
                missing_inst = [p for p in missing_inst if not _public_url_available(_public_image_url_for_buffer(p, state))]
                if not missing_inst:
                    break
                print(f"   Aguardando imagens públicas Instagram... Tentativa {attempt + 1}/15 (Restantes: {len(missing_inst)})")
                time.sleep(3)
                
            if missing_inst:
                print("[AVISO] Algumas imagens do Instagram não estão públicas ainda. O Buffer poderá falhar no upload.")
            else:
                print("-> Sucesso: Todas as imagens do Instagram estão públicas!")
        ...
        carousel_time = "2026-06-15T21:00:00+01:00"
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            update_final_post_copy(
                                state.final_posts_path,
                                post_id=p.post_id,
                                notes="[2026-06-15] Agendado no Instagram às 21:00."
                            )
                            break
        ...
        print(f"-> [AVISO] Menos de 4 posts de Instagram encontrados ({len(instagram_posts_sorted)}). A saltar carrossel.")
    ...
    print("\n=== TABELA DE RESUMO DE AGENDAMENTO (2026-06-15) ===")
