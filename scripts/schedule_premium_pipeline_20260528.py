...
# Dicionário com os novos corpos de texto otimizados e curtos para o X (limite 280 caracteres)
X_POLISHED_BODIES = {
    "post_8ecd71af2329494cb9": "A OpenAI, a Thrive e a Crete criaram um agente de IA auto-aperfeiçoável para a área fiscal usando o Codex, automatizando declarações e acelerando fluxos de trabalho.",
    "post_85573f666492af9c24": "A adoção de agentes de IA a nível empresarial está a crescer rapidamente, mas há uma desconexão evidente entre a ambição e a execução, pois a maioria das organizações ainda carece de processos e design organizacional adequados para suportar esta transição.",
    "post_3fa7031ab9774d7188": "Luís Dinis, diretor da Tabaqueira, aponta a IA como um agente de mudança estrutural nas empresas, otimizando processos industriais e gerando valor real para cidadãos e negócios.",
    "post_34be0133925ba4c73f": "A Gartner avisa que aplicar uma governação uniforme e indiferenciada a todos os agentes de IA, sem distinguir a sua autonomia e acesso, aumentará o risco de falhas graves nas empresas."
}

PLAN = [
    ("topic_16413b1d109bedaeea", "2026-05-28T09:00:00+01:00"),
    ("topic_45c61fc2740eb534ec", "2026-05-28T13:00:00+01:00"),
    ("topic_45288f3fafaa25d8aa", "2026-05-28T16:00:00+01:00"),
    ("topic_da9b19a1174b9ba379", "2026-05-28T21:00:00+01:00"),
]
...
def apply_x_polish_replacements(posts_path: Path) -> None:
    ...
    for line in posts_path.read_text(encoding="utf-8").splitlines():
        ...
        if post_id in X_POLISHED_BODIES:
            ...
            post["body"] = new_body
            post["editor_notes"] = post.get("editor_notes", "") + f"\n[{datetime.now().isoformat()}] Polimento manual do X aplicado para conformidade de 280 caracteres."
            modified_count += 1
...
def main() -> None:
    print("=== INICIANDO AGENDAMENTO EDITORIAL PREMIUM (2026-05-28) ===")
    state = DashboardState(ROOT / "data")
    
    # 1. Efetuar Backup da base de dados local
    backup_path = state.final_posts_path.with_name(
        f"final_posts.jsonl.bak_before_schedule_20260528_{datetime.now():%Y%m%d_%H%M%S}"
    )
    ...
    combined_body = "\n\n".join(paragraphs)
    combined_hashtags = "#InteligenciaArtificial #IA #Produtividade #Negocios #Gestao #Governanca #Portugal"
    ...
    carousel_time = "2026-05-28T21:00:00+01:00"
    ...
    for p in instagram_posts_sorted:
        ...
        update_final_post_copy(
            state.final_posts_path,
            post_id=p.post_id,
            notes="[2026-05-27] Agendado como parte do Carrossel Combinado de 4 slides das 21:00."
        )
        scheduled_summary.append({
            "post_id": p.post_id,
            "channel": "instagram",
    ...
    print("\n=== TABELA DE RESUMO DE AGENDAMENTO (2026-05-28) ===")
